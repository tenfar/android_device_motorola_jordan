...
void clear_hotplug()
{
	...
	write(ofd, "", 1);
    printf("exec clear_hotplug(0) functioning...");
	close(ofd);
}
...
int main(int argc, char **argv, char **env)
{
	...
	if (geteuid() == 0) {
		clear_hotplug();
		/* remount /system rw */
		if (mount("/dev/block/mmcblk1p21", "/system", "ext3", MS_REMOUNT | MS_NOSUID | MS_NODEV | MS_NOATIME | MS_NODIRATIME , 0) < 0)
			mount("/dev/block/mmcblk1p21", "/system", "ext3", MS_REMOUNT | MS_NOSUID | MS_NODEV | MS_NOATIME | MS_NODIRATIME , 0);
