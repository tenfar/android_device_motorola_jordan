...
hijack_log("    hijack_umount(%s, %s) executing...", "/sbin/hijack", "/pds");
                result = hijack_umount("/sbin/hijack", "/pds");
hijack_log("      returned: %d", result);
...
#ifdef LOG_ENABLE
		char * init_args[] = { "/init", LOG_PATH, NULL };
hijack_log("    exec(\"%s %s\") executing...", "/init", LOG_PATH);
#else
                char * init_args[] = { "/init", NULL };
hijack_log("    exec(\"%s\") executing...", "/init");
#endif
